...
The sample loading order on the RT plate are stored at this [spreadsheet](/data/'9_6_24 12 OFC for ScaleRNA yield summary.xlsx').
...
[runParams.yml](/blob/master/script/runParams.yml)
...
[DEG counts in each cell type between iso and soc groups](/blob/master/outputs/DEG_FC0.3_counts_barplot_2.pdf)
...
* [split the integrated .bam files into individual cell based on the barcode information](/blob/master/script/workflow.sh)

* [To gather the single-cell .bam files to the same folder annotated as the same cell type (not necessarily move to same folder, create softlink works)](/blob/master/script/run_bam_gather.sh)

* [Merge the .bam files together for each cell type in each sample](/script/samtools_merge.sh)

* [Make .bigwig from the merged .bam file](/blob/master/script/make_bigwig.sh)

* [make Genome tracks with pygenometrack](/blob/master/script/soc_FvsM_pygenometrack.sh)
